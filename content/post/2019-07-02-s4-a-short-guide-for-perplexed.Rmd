...
S4 is a formal object oriented system in R. It's implemented in the
...
Community is also a driver for using S4. The Bioconductor
project uses S4 extensively: there are standard classes in the
ecosystem to represent many types of 'omics' data. Developers and users alike 
can reuse these classes and their associated methods without the need to 
invent their own and enables interoperability between many different packages.
...
The design of an S4 class is merely a way of setting up an abstraction for a data analysis
...
okTurtle <- function(object) {
  ...
  if (loc_ln != 2) return("location must have length two.")
  
  # check orientation is between -360 to 360 and has length one
  orientation <- object@orientation
  if (length(orientation) != 1) return("orientation must have length one.")
  if (abs(orientation) > 360) 
    return(paste("orientation angle", 
          orientation, 
          "must be between -360 and 360 degrees."))
  # check path is a matrix
  path <- object@path
  if (!is(path, "matrix")) return("path must be a matrix")
  if (ncol(path) != 2) return("path must have two columns")
  TRUE
}
...
Now we also need an interface to creating a new object of class Turtle, called
a constructor. Generally, this is a function that calls `new` and has 
arguments corresponding to the slots of our object.
...
## Principle 2: in statistics we like methods
...
Similarly, we can implement a generic for reorienting.
```{r, message = FALSE}
...
of implementing an interface like setters or getters. 
...
* Reusing classes and methods enable a user and developer to minimise code de-duplication via inheritance.
